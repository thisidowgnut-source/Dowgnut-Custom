...
export function ShopHome() {
  ...
  return (
    <div className="relative flex h-full w-full flex-1 flex-col items-center justify-between px-4 py-1 overflow-hidden select-none">
      ...
      {/* 3 BIG showcase display donuts — tightly spaced & layered, zero scroll */}
      <div className="relative z-10 flex w-full max-w-sm flex-1 flex-col items-center justify-center -space-y-5 sm:-space-y-7 my-auto overflow-visible py-0">
        {TYPES.map((t, i) => {
          ...
          return (
            ...
            >
              {/* Ground shadow underneath spinning donut */}
              <div className="absolute bottom-1 h-4 w-44 sm:w-56 rounded-full bg-black/12 blur-md transition-transform duration-300 group-hover:scale-110 pointer-events-none" />

              <motion.img
                src={imgSrc}
                alt={t.label}
                className="size-48 sm:size-56 md:size-64 object-contain drop-shadow-2xl transition-transform duration-200"
                draggable={false}
                animate={{ rotate: 360 }}
